...
describe('get-signed-url', () => {
  let S3Spy
  beforeEach(() => {
    S3Spy = jest.spyOn(S3.prototype, 'getSignedUrlPromise')
  });


  it('returns upload URL with valid request', async () => {
    // Arrange
  ...
  describe(`request validation`, () => {
    it('returns 500 error with missing file name', async () => {
      // Arrange
      const event = constructAPIGwEvent({}, {
        method: 'POST',
        requestContext: {
      ...
      const parsedResultBody = JSON.parse(result.body)
      expect(parsedResultBody).toEqual({
        message: "Error: No file name specified"
      })
    });

    it('returns 500 error with missing file extension', async () => {
      // Arrange
      const event = constructAPIGwEvent({}, {
        method: 'POST',
        requestContext: {
      ...
      const parsedResultBody = JSON.parse(result.body)
      expect(parsedResultBody).toEqual({
        message: "Error: File Extension not specified"
      })
    });

    it('returns 500 error with invalid file extension', async () => {
      // Arrange
      const event = constructAPIGwEvent({}, {
        method: 'POST',
        requestContext: {
